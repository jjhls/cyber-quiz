...
export default function AdminQuestions() {
  ...
  const columns = [
    { title: 'ID', dataIndex: 'id', key: 'id', width: 80, render: (v: string) => <span className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{v.slice(-6)}</span> },
    { title: '题目', dataIndex: 'title', key: 'title', ellipsis: true, render: (v: string) => <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>{v}</span> },
    ...
    { title: '题型', dataIndex: 'type', key: 'type', width: 80, render: (v: string) => <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{({ single: '单选', multiple: '多选', truefalse: '判断', fillblank: '填空' } as Record<string, string>)[v]}</span> },
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <Title level={3} className={`!mb-0 ${isDark ? '!text-slate-100' : '!text-slate-900'}`}>📝 题目管理</Title>
        <Space>
          <Button icon={<ImportOutlined />} className={`rounded-xl ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-700'}`}>
            批量导入
          </Button>
        ...
        <Form form={form} layout="vertical" className="mt-4">
          <div className="grid grid-cols-3 gap-4">
            <Form.Item name="category" label="分类" rules={[{ required: true }]}>
              <Select options={categoryOptions} className={inputClass} />
            </Form.Item>
            <Form.Item name="difficulty" label="难度" rules={[{ required: true }]}>
              <Select options={difficultyOptions} className={inputClass} />
            </Form.Item>
            <Form.Item name="type" label="题型" rules={[{ required: true }]}>
              <Select options={typeOptions} className={inputClass} />
            </Form.Item>
          </div>

          <Form.Item name="title" label="题目内容" rules={[{ required: true }]}>
            <Input.TextArea rows={3} className={inputClass} />
          </Form.Item>
            ...
            {({ getFieldValue }) => {
              ...
              return showOptions ? (
                <Form.Item name="options" label="选项（每行一个，用逗号分隔）">
                  <Input placeholder="A. 选项一, B. 选项二, C. 选项三, D. 选项四" className={inputClass} />
                </Form.Item>
              ) : null;
            }}
          </Form.Item>
          ...
          <Form.Item name="answer" label="正确答案" rules={[{ required: true, message: '请填写正确答案' }]}>
            <Input placeholder="单选题填选项字母，多选题用逗号分隔，填空题用逗号分隔多个答案" className={inputClass} />
          </Form.Item>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Form.Item name="tags" label="标签">
              <Input placeholder="用逗号分隔，如: SQL注入, 注入攻击" className={inputClass} />
            </Form.Item>
