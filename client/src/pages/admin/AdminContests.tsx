...
export default function AdminContests() {
  ...
  const columns = [
    { title: '名称', dataIndex: 'title', key: 'title', ellipsis: true, render: (v: string) => <Text className="text-slate-300">{v}</Text> },
    { title: '状态', dataIndex: 'status', key: 'status', width: 100, render: (v: string) => <Tag color={statusMap[v]?.color}>{statusMap[v]?.label}</Tag> },
    { title: '题目', key: 'questions', width: 80, render: (_: any, r: Contest) => <Text className="text-slate-400">{r.questionIds?.length || 0}题</Text> },
    { title: '时长', dataIndex: 'duration', key: 'duration', width: 80, render: (v: number) => <Text className="text-slate-400">{v}分钟</Text> },
    { title: '总分', dataIndex: 'totalScore', key: 'totalScore', width: 80, render: (v: number) => <Text className="text-blue-400">{v}分</Text> },
    { title: '开始时间', dataIndex: 'startTime', key: 'startTime', width: 180, render: (v: string) => <Text className="text-slate-500 text-xs">{new Date(v).toLocaleString('zh-CN')}</Text> },
    {
      title: '操作',
      key: 'action',
      width: 120,
        ...
        <Form form={form} layout="vertical" className="mt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Form.Item name="startTime" label="开始时间" rules={[{ required: true }]}>
              <DatePicker showTime className="w-full bg-slate-800" />
            </Form.Item>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Form.Item name="shuffleQuestions" label="随机题目顺序" valuePropName="checked">
              <Select options={[{ value: true, label: '是' }, { value: false, label: '否' }]} className="bg-slate-800" />
            </Form.Item>
